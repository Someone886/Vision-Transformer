...
class ViT(nn.Module):
    ...
    def patchify(self, x, patch_size):
        ...
        ################################
        ## 1. Reshape/Split input into patches
        ## 2. Transpose into correct order if required
        ## 3. Reshape/Flatten input into linear patch
        #################################
        out = x
    ...
    def __call__(self, x, train=True):
        ...
        """
        ############################################
        ## 1. Convert input image into flattened linear patched image
        ## 2. Convert flattened image into image embeddings
        ## 3. Prepend CLS token to embeddings
        ## 4. Add position embeddings
        ## 5. Pass through the transformer encoder
        ## 6. Take layer norm of the output of the transformer encoder
        ## 7. Pass the class token to the linear head to produce logits
        ################################################
